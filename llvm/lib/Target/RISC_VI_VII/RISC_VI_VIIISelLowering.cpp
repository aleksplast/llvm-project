...
#include "RISC_VI_VIIGenCallingConv.inc"
...
SDValue RISC_VI_VIITargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_RISC_VI_VII);
  ...
  Chain = DAG.getNode(RISC_VI_VIIISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_RISC_VI_VII);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const RISC_VI_VIISubtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const RISC_VI_VIITargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// RISC_VI_VII formal arguments implementation
SDValue RISC_VI_VIITargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_RISC_VI_VII);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &RISC_VI_VII::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    RISC_VI_VIIFunctionInfo *UFI = MF.getInfo<RISC_VI_VIIFunctionInfo>();
...
bool RISC_VI_VIITargetLowering::CanLowerReturn(
  ...
  if (!CCInfo.CheckReturn(Outs, RetCC_RISC_VI_VII))
    return false;
...
SDValue
RISC_VI_VIITargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
  ...
  const RISC_VI_VIISubtarget &STI = MF.getSubtarget<RISC_VI_VIISubtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_RISC_VI_VII);
  ...
  return DAG.getNode(RISC_VI_VIIISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue RISC_VI_VIITargetLowering::PerformDAGCombine(SDNode *N,
...
bool RISC_VI_VIITargetLowering::isLegalAddressingMode(const DataLayout &DL,
